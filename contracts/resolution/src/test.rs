...
#[test]
fn deposit_collateral_records_balance() {
    let env = Env::default();
    let (client, _, _) = setup(&env);
    let token = make_token(&env);
    let proposer = Address::generate(&env);
    StellarAssetClient::new(&env, &token).mint(&proposer, &1_000);

    client.deposit_collateral(&proposer, &token, &500);
    assert_eq!(client.get_proposer_collateral(&proposer), 500);

    client.deposit_collateral(&proposer, &token, &200);
    assert_eq!(client.get_proposer_collateral(&proposer), 700);
}

#[test]
fn deposit_collateral_rejects_zero_or_negative() {
    let env = Env::default();
    let (client, _, _) = setup(&env);
    let token = make_token(&env);
    let proposer = Address::generate(&env);
    assert_eq!(
        client.try_deposit_collateral(&proposer, &token, &0),
        Err(Ok(ContractError::InvalidCollateral))
    );
    assert_eq!(
        client.try_deposit_collateral(&proposer, &token, &(-1)),
        Err(Ok(ContractError::InvalidCollateral))
    );
}

#[test]
fn slash_collateral_transfers_and_zeroes_balance() {
    let env = Env::default();
    let (client, admin, _) = setup(&env);
    let token = make_token(&env);
    let proposer = Address::generate(&env);
    ...
    StellarAssetClient::new(&env, &token).mint(&proposer, &1_000);
    client.deposit_collateral(&proposer, &token, &1_000);

    let slashed = client.slash_collateral(&admin, &proposer, &token, &recipient);
    assert_eq!(slashed, 1_000);
    assert_eq!(client.get_proposer_collateral(&proposer), 0);
...
fn slash_collateral_rejects_non_admin() {
    ...
    let rando = Address::generate(&env);
    assert_eq!(
        client.try_slash_collateral(&rando, &proposer, &token, &proposer),
        Err(Ok(ContractError::NotAdmin))
    );
}

#[test]
fn slash_collateral_rejects_empty_balance() {
    let env = Env::default();
    let (client, admin, _) = setup(&env);
    let token = make_token(&env);
    let proposer = Address::generate(&env);
